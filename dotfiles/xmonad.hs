...
myLayout =
  avoidStruts
   ...
   where
     ...
     dev1Layout = onWorkspace dev1Ws (tiled ||| full)
     dev2Layout = onWorkspace dev2Ws (tiled ||| full)
     nixLayout = onWorkspace nixWs (tiled ||| full)
...
myWS :: [WorkspaceId]
myWS = [webWs, dev1Ws, dev2Ws, nixWs, comWs, wrkWs]
...
projects =
  [ Project { projectName      = webWs
            , projectDirectory = "~/"
            , projectStartHook = Just $ spawn "firefox -P 'default' --new-window"
            }
  , Project { projectName      = dev1Ws
            , projectDirectory = "~/"
            , projectStartHook = Just $ do spawn myTerminal
                                           spawn myTerminal
            }
  , Project { projectName      = dev2Ws
            , projectDirectory = "~/"
            , projectStartHook = Nothing
            }
  , Project { projectName      = nixWs
