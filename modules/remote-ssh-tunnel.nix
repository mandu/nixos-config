...
let
  ...
  cfg = config.services.remote-ssh-tunnel;
in

{
  ...
  options = {
    services.remote-ssh-tunnel = {
      enable = mkOption {
        ...
        default = false;
        description = ''
          Whether to enable a "remote-ssh-tunnel" reverse SSH proxy.
        '';
      };
      ...
      persist = mkOption {
        ...
        description = ''
          ...
          set this option to false (or not use <literal>remote-ssh-tunnel</literal>
          at all).
        '';
      };
  ...
  config = mkIf cfg.enable {
    systemd.services.remote-ssh-tunnel =
    {
      description = ''
      ...
      script = with cfg;  ''
        ${openssh}/bin/ssh -NTC -o ServerAliveInterval=30 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=no -R ${toString bindPort}:localhost:22 -l ${remoteUser} -p ${toString remotePort} ${remoteHostname}
      '';
    };
  };
}
